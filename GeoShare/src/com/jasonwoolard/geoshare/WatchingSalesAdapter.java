...
public class WatchingSalesAdapter extends ParseQueryAdapter<Sale> {

	static String mTAG = "WatchingSalesAdapter";	

	public WatchingSalesAdapter(Context context) {
		super(context, new ParseQueryAdapter.QueryFactory<Sale>() {
			@SuppressWarnings({ "unchecked", "rawtypes" })
			public ParseQuery<Sale> create() {
			    ParseUser mCurrentUser = ParseUser.getCurrentUser();
				ParseRelation<ParseObject> mWatchingRelation = mCurrentUser.getRelation("Watching");
				ParseQuery query = mWatchingRelation.getQuery();
				query.orderByDescending("createdAt");
				try {
					int watchingAmount = query.count();
					WatchingSalesActivity.setTextView(watchingAmount);
